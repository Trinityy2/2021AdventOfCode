...
def _find_paths(paths, possible_paths, curr_path, next_path: str, double = False):
    """ Next path is just a string of the key to the next path we're trying out, double means we've used a small cave twice """
    ...
    if next_path == 'start':
        ...
        if next_path.islower():
            # Can only traverse twice once
            if next_path in curr_path and double:
                return

    # If we reach this part, it means we can start doing the rest
    _double = double
    if next_path.islower() and next_path in curr_path:
        _double = True
    curr_path.append(next_path)
    for check_path in paths[next_path]:
        _find_paths(paths, possible_paths, curr_path, check_path, _double)
    curr_path.pop(-1)
